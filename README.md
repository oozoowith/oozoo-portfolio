...
브랜드 경험 기획자 정은용의 포트폴리오 웹사이트. `index.html`(SPA, CSS/JS 인라인) + 프로젝트별 정적 페이지(`generate.mjs`가 생성, 진짜 URL·SEO용). 상세 구조는 `HANDOFF.md`(특히 10절) 참고.

## 구조
- **Home** — 좌측 이름 2줄(정은용 / Eunyong Chung) + 우측 소개·구분선·링크 + 스킬 4카드
- **About** — 소개 + 경력/학력/논문·인터뷰
- **Works** — 갤러리(커버 + 번호 + 제목 + 한 줄) + 주제 필터 + 클릭 시 상세(진짜 URL `/works/{id}`)
- **Playground** — 개인 프로젝트 4개(마음의 실루엣·씨앗실험실·TMT TOWN·널 위한 문화예술)
- **Contact** — 이메일(클립보드 복사)
